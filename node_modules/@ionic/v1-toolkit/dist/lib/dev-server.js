...
async function createDevServerHandler(options) {
    const devServerConfig = {
        consolelogs: options.consolelogs,
        wsPort: options.devPort,
    };
    const devServerJs = await utils_fs_1.readFile(path.join(__dirname, '..', '..', 'assets', 'dev-server.js'), { encoding: 'utf8' });
    return (req, res) => {
...
async function attachDevServer(app, options) {
    app.get(`/${exports.DEV_SERVER_PREFIX}/dev-server.js`, await createDevServerHandler(options));
}
...
async function createLiveReloadServer({ host, port, wwwDir }) {
    const tinylr = await Promise.resolve().then(() => require('tiny-lr'));
...
async function createDevLoggerServer(host, port) {
    const WebSocket = await Promise.resolve().then(() => require('ws'));
