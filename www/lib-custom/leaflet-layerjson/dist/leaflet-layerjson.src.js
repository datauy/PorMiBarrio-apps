...
(function() {
  ...
  L.LayerJSON = L.FeatureGroup.extend({
    ...
    options: {
      ...
      locAsArray: false, /// interpret location data as a raw array [lat,long,prop1,prop2,....propi ...propN]
      ...
      onEachFeature:null,//function  onEachFeature of L.geoJson
    ...
    initialize: function(options) {
      ...
      this._onEachFeature =  this.options.onEachFeature || this._onEachFeatureDefault ;
    ...
    _defaultDataToMarker: function(data, latlng) { //make marker from data
      ...
      var title = this._getPath(data, this.options.propertyTitle),
        markerOpts = L.Util.extend({
          icon: this._buildIcon(data, title),
          title: title
        }, data),
    ...
    addMarker: function(data) {
      ...
      if (!L.Util.isArray(data) || !data.length) return;

      if (this.options.locAsArray) {
        latlng = L.latLng(parseFloat(data[0]), parseFloat(data[1]));
    ...
    update: function() { //populate target layer
      ...
      this._curReq = this._callData(bbox, function(json) {
        ...
        if (that.options.propertyItems)
          ...
          if(that.options.invertCoordinates &&json.features){
        ...
        that.fire('dataloaded', {
          data: json
        });

        if (that.options.locAsGeoJSON) {
          that.addFeatureCollection.call(that,json);
            //

        } else {
          for (var k in json)
          //TODO sacar esto cuando se devuelva geojson
            if (k == "pins") {
            var pins = json[k];
            for (var i = 0; i < pins.length; i++) {
              that.addMarker.call(that, pins[i]);
            }
          } else {
    ...
    addFeatureCollection:function(geoJson){
      var layer ;
      if(geoJson.type==="FeatureCollection" && geoJson.features){
        layer = L.geoJson(geoJson,{onEachFeature:this._onEachFeature});
        this.addLayer(layer);
      }
    },
